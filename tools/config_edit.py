...
def parse_var(s):
    ...
    if len(items) < 2:
        raise ValueError(
            "invalid --set item {!r}: expected [SECTION:]KEY=VALUE".format(s))
    # rejoin the rest:
    value = '='.join(items[1:])
...
def apply_edits(updater, values):
    """Apply parsed section/key/value edits to an open ConfigUpdater.

    A value of ``comment_out`` comments the key rather than setting it.
    Keys under the ``_global_`` pseudo-section are applied to every section
    that already defines that option.
    """
    for sec in values:
        if sec == '_global_':
            continue
...
def main(argv=None):
    ...
    args, u = ap.parse_known_args(argv)
    args = vars(args)
